...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        x_veh = np.ones((4, 1))
        x_veh[0:3] = x[0:3]
        x_sens = (self.veh_to_sens @ x_veh)[0:3]
        alpha = np.arctan2(x_sens[1, 0], x_sens[0, 0])
    ...
    def get_hx(self, x):
        ...
        if self.name == 'lidar':
            ...
            pos_veh = np.ones((4, 1))  # homogeneous coordinates
            pos_veh[0:3] = x[0:3]
            pos_sens = (self.veh_to_sens * pos_veh)[0:3]  # transform from vehicle to camera coordinates

            if pos_sens[0] == 0:
                raise ValueError("p_x must not be 0")
